...
{{--     
   ...
   {{-- @include('components.Topbar') --}}
      ...
      @include('components.ContentExamenJob')
...
$('#note_section').hide();
 ...
 //console.log(question.id);
  ...
  function getNote() {
    ...
    $.ajax({
      url: '{{route('note_job', $fascicule->id )}}',
      type: 'GET',
      dataType: 'json',
...
function deleteComposer() {
  ...
  var reloadButton = $('#reloadButton');
    ...
    $.ajax({
      url: '{{route('composer_job_delete', $fascicule->slug )}}',
      type: 'GET',
      dataType: 'json',
      success: function(response) {
...
function getQuestions() {
  ...
  var reloadButton = $('#reloadButton');
    ...
    $.ajax({
      url: '{{route('take_job', $fascicule->id )}}',
      type: 'GET',
      dataType: 'json',
      success: function(response) {
        loadingIndicator.hide();
        ...
        if (currentQuestionIndex < questions.length && resultat === true) {
        ...
        else if(currentQuestionIndex >= questions.length && resultat === false)
         {
          ...
          $('#all').html('<div class="col-lg-12 d-flex align-items-center justify-content-center">'+
                         '<a href="{{route('resultat_job', $fascicule->slug)}}" class="col-sm-10 col-xs-10 col-md-4 justify-content-center align-items-center" id="suivant" style="background-color: rgb(88, 16, 197); text-decoration:none; padding:15px; color:white; text-align:center; border:none">'+'Cliquez ici pour voir la correction'+'</a>'+'</div>'
                                );
