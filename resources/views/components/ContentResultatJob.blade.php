...
@endphp
  <div class="container" id="all" style="transition: all 0.5s">
    <h2 style="color: rgb(88, 16, 197); font-weight:700;" id="tit">{{ucfirst($fascicule->intitule)}}</h2>
...
@endphp
        <tr>
            ...
            <td style="@if($reponses[$item]->reponse == 'Totalement' || $reponses[$item]->reponse == 'Totally') color:green @else color:red @endif">@if($reponses[$item]->reponse == 'Pas du tout' || $reponses[$item]->reponse == 'No way') @lang('message.pas_du_tout') @else {{ucfirst($reponses[$item]->reponse)}} @endif</td>   
            <td> @lang('message.totalement') </td>
        </tr>
...
{{--  --}}
    ...
    <button class="col-sm-5 col-xs-5 col-md-2" id="suivant" style="background-color: rgb(88, 16, 197); text-decoration:none; padding:10px; color:white; text-align:center; border:none">Imprimer</button> 
